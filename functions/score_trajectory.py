...
from functions.src import scoring
...
def read_score_trajectory(pdb_file, score_list, src_route, path, chain_join, binder):
    ...
    for i, line in enumerate(PDB_traj):
        if line == "ENDMDL" or i == len(PDB_traj) - 1:
            ...
            # Perform scoring and save file only every 100th model
            if model_number % 100 == 0:
                # Save the PDB file
                output_file = open(f"model{model_number}.pdb", "w")
                output_file.write(new_file_text.rstrip('\r\n'))  # rstrip to remove trailing newline
                output_file.close()
                
                # Change residue names
                os.system(f"sed -i 's/HIP/HIS/g' model{model_number}.pdb")
                os.system(f"sed -i 's/HID/HIS/g' model{model_number}.pdb")
                os.system(f"sed -i 's/GLH/GLU/g' model{model_number}.pdb")
                os.system(f"sed -i 's/ASH/ASP/g' model{model_number}.pdb")
                os.system(f"sed -i 's/CD  ILE/CD1 ILE/g' model{model_number}.pdb")
                
                # Model ID
                p = f"model{model_number}"
                ...
                # Remove the saved PDB file
                os.system(f"rm {p}.pdb")
                
            # Increment the model counter and reset the text for the next model
            model_number += 1
            new_file_text = ""
            
            # Stop processing if there are more lines after ENDMDL
            if i < len(PDB_traj) - 1 and not PDB_traj[i + 1].startswith("MODEL"):
                break  # Exit loop if the next line is not a new model
...
#     src_route="openm/PARCE" # Local path of your PARCE installation folder
...
#     traj_path='openm/md_run/output_docked/' #"/home/PARCE-1/auxiliar_scripts/scoring_trajectories"
#     # Name of the PDB trajectory file or single file
#     pdb_name=f'system_1_0_traj_reimaged'
